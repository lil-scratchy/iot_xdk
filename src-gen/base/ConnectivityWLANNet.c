...
#define NETWORK_SSID  "EventiLoccioni"
#define NETWORK_PSK  "welcometoloccioni"

static void ConnectivityWLANNet_WlanConnectStatusCallback(WlanNetworkConnect_Status_T connectStatus)
{
	BCDS_UNUSED(connectStatus);
	(void) xSemaphoreGive(WlanEventSemaphore);
}
static void ConnectivityWLANNet_NetworkIpConfigStatusCallback(WlanNetworkConfig_IpStatus_T ipStatus)
{
...
Retcode_T ConnectivityWLANNet_Enable(void)
{
	...
	exception = WlanNetworkConnect_Init(ConnectivityWLANNet_WlanConnectStatusCallback);
	...
	exception = WlanNetworkConfig_SetIpDhcp(ConnectivityWLANNet_NetworkIpConfigStatusCallback);
	if(exception == NO_EXCEPTION)
	{
